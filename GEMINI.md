...
*   **Scope:** This repository contains the Flutter client only. The backend is a separate Laravel/PostgreSQL repository. The mobile app must consume ONLY the mobile subset of the API (auth, coletas, sitios, sync, notificacoes, usuarios/me), avoiding administrative rules (curadorias, auditorias). Note that "coletas" represent raw field data captured on mobile, while "bens_materiais" are consolidated ecosystem records. The app focuses on capture, local editing, and synchronization.
